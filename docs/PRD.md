...
**Ключевое отличие от CLI-подхода**: Claude Code сам является оркестратором. Вместо отдельных команд (`stitch generate`, `stitch edit`) пользователь ведёт диалог на естественном языке, а скиллы учат Claude правильно использовать 14 MCP-инструментов Stitch.
...
│   ├── stitch-upload/SKILL.md   # Загрузка wireframe/скриншотов
...
| UC-5 | Загрузка wireframe/скриншота для оцифровки | stitch-upload |
| UC-6 | Дизайн-система (единый стиль) | stitch-design (Step 5) |
| UC-7 | Настройка API key | /stitch-setup |
| UC-8 | Сброс настроек | /stitch-reset |
